...
def load_observation(example_dir: Path, load_depth: bool = False) -> Tuple[np.ndarray, Union[None, np.ndarray], CameraData]:
...
def load_observation_tensor(example_dir: Path, load_depth: bool = False) -> ObservationTensor:
...
def save_predictions(example_dir: Path, pose_estimates: PoseEstimatesType) -> None:
...
def run_inference(example_dir: Path, model_name: str) -> None:
